...
(function() {
	...
	function reqVerifyVer() {
		...
		Net.asynConnect("base/version/verify", req, callback);
	...
	function checkSessionByEchoTest(){
		if (systemInfo == null) {
			Global.getSystemInfo(getInfo);
		}else{
			check();
		}
		...
		function check(){
			if(systemInfo != null){
				...
				Scene.alert("JSLOG,checkSessionByEchoTest------------");
				Net.asynConnect("base/echo-test", req, checkBack);
			}			
		}
		
		function checkBack(data){
			if(data.responseCode !="0"){
				setTimeout(checkSessionByEchoTest, 1000);
			}				
		}		
	}
