//app/TransactionManage / TransactionManageIndex.js
define(['Moo'], function(Moo) {
	var req_loadMore = {};
	var singleResearchTag = false;

	var TransactionManageIndex = new Class({

		initialize : function() {

		},

		handleResFromReqRecord : function(msg) {
			...
			if ((null != window.TransactionManageIndex.params && undefined != window.TransactionManageIndex.params) && (null != window.TransactionManageIndex.params.startDate && null != window.TransactionManageIndex.params.endDate)) {
		...
		handleRecordData : function(params) {
		...
		getTransTypeDesc : function(transType) {
			// 交易类型
			// 1021 	消费
			// 3021 	消费撤销
			// 3051 	退货
		...
		getOrderStateDesc : function(orderState) {
			// 		订单状态
			// 0	成功
			// 1	失败
			// 2	已冲正
		...
		gotoSingleRecord : function(data) {
		...
		gotoConsumptionRecord : function(searchData) {
			var req;
