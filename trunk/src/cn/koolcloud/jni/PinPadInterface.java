...
* android.permission.KOOLCLOUD_PINPAD
* android.permission.KOOLCLOUD_PIN_GET_PIN_BLOCK
* android.permission.KOOLCLOUD_PIN_MAC
* android.permission.KOOLCLOUD_PIN_ENCRYPT_DATA
* android.permission.KOOLCLOUD_PIN_UPDATE_USER_KEY
* android.permission.KOOLCLOUD_PIN_UPDATE_MASTER_KEY
*/
...
public class PinPadInterface
{
    ...
    static
	{
		...
		System.loadLibrary("koolcloud_pinpad"); 
 }

	/** key code reference definition, better to define in high-tier class 
	...
	/**
	 * select master key and user key (usually, one device has only one master key, do not need consider multiple master keys) 
	 * @param nKeyType : 0 : dukpt, 1: Tdukpt, 2 : master key, 3 : public key, 4 : fix key
	 * @param nMasterKeyID : master key ID , [0x00, ..., 0x09], make sense only when nKeyType is master-session pair,
	 * @param nUserKeyType : user key ID, [0x00, 0x01], 0: pin key, 1: encrypt key
	 * @param nAlgorith : 1 : 3DES, 0 : DES
	 * return value < 0 : error code
	 * 			   >= 0 : success (suggest 0)
	 */
    public native static int selectKey(int nKeyType, int nMasterKeyID, int nUserKeyType, int nAlgorith);
		
	/**
	 * set the maximal length of pin
	 * @param nLength : length >= 0 && length <= 0x0D
	 * @param nFlag : 1 : maximal length, 0 : minimal length
	 * return value < 0 : error code
	 * 			   >= 0 : success (suggest 0)
	 */
	...
	/**
	 * encrypt string using user key (common encrypt method, for MAC, better to invoke calculateMac())
	 * @param arryPlainText : plain text
	...
	/**
	 * calculate the MAC using current user key (real implementation is required)
	 * @param arryData : data
	 * @param nDataLength : data length
	 * @param nMACFlag : 0 : X99, 1 : ECB
	 * @param arryMACOutBuffer : MAC data buffer
	 * return value < 0 : error code
	 * 			   >= 0 : success, length of MAC data buffer
	 */
	 ...
	 * @param nMasterKeyID : master key ID
	 * @param arrayOldKey : old key
	 * @param nOldKeyLength : length of old key
	 * @param arrayNewKey : new key
