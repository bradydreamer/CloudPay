...
public class MessageType implements Constant
{
  public static MessageTypeTable[] messageTypeTable = {
      ...
      new MessageTypeTable(TRAN_EC_CASH_SAVING,			"0200", "630000"),  // 36  现金充值
      new MessageTypeTable(TRAN_EC_VOID_SAVING,			"0200", "170000"),  // 37  现金充值撤销
      new MessageTypeTable(TRAN_EC_LOAD,				"0200", "600000"),  // 38  指定账户圈存
      new MessageTypeTable(TRAN_EC_LOAD_NOT_APPOINTED,	"0200", "620000"),  // 39  非指定账户圈存
  	  // 磁条卡充值
      ...
      new MessageTypeTable(TRAN_BATCH_UPLOAD_MAG_OFFLINE,       "0320", "      "),	// 50
      new MessageTypeTable(TRAN_BATCH_UPLOAD_PBOC_OFFLINE_SUCC, "0320", "      "),	// 51
      new MessageTypeTable(TRAN_BATCH_UPLOAD_MAG_ONLINE,        "0320", "      "),	// 52
      new MessageTypeTable(TRAN_BATCH_UPLOAD_MAG_ADVICE,        "0320", "      "),	// 53
      new MessageTypeTable(TRAN_BATCH_UPLOAD_PBOC_ADVICE,       "0320", "      "),	// 54
      new MessageTypeTable(TRAN_BATCH_UPLOAD_PBOC_ONLINE,       "0320", "      "),	// 55
      new MessageTypeTable(TRAN_BATCH_UPLOAD_PBOC_OFFLINE_FAIL, "0320", "      "),	// 56
...
/*      
      //自定义  快捷支付
      new MessageTypeTable(TRAN_SALE_9121,           	"0200", "000000"),  // 65
      new MessageTypeTable(TRAN_SALE_9100,           	"0200", "000000"),  // 66
      new MessageTypeTable(TRAN_SALE_9110,           	"0200", "000000"),  // 67
      new MessageTypeTable(TRAN_SALE_9130,           	"0200", "000000"),  // 68
      new MessageTypeTable(TRAN_SALE_9140,           	"0200", "000000"),  // 69
      */
  ...
  public static int getTransType(String procCode, byte[] reqMsg){
	  ...
	  for (i = 0; i < messageTypeTable.length; i++){
		  ...
		  String reqMsgType_s = StringUtil.toString(messageTypeTable[i].reqMsgType);
		  if(reqMsg_s.equals(reqMsgType_s)  ){
			  String processingCode_s = StringUtil.toBestString(messageTypeTable[i].processingCode);
			  System.err.println("procCode = " + procCode + ", processingCode_s = " + processingCode_s);
