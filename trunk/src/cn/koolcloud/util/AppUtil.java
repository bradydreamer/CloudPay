...
public class AppUtil
{
  ...
  *
  * @param PhoneString The value to be checked.
  *
  */  
  ...
  /**
    ...
    *
    * @param timeData The int value to be checked.
    *
    */  
	...
	public static int toAmount(String strAmount)
	{
		...
		for(int i=0;  i<temp.length; i++)
		{
			if(   strAmount.getBytes()[i] != '.'
			   && (   strAmount.getBytes()[i] < '0'
				   || strAmount.getBytes()[i] > '9'
			      )
			  ) 
			{
				return -1;
	...
	public static byte[] toCurrency(long number, boolean bcdFlag)
	{
		...
		{
			currency = new byte[6];
			currency = StringUtil.hexString2bytes(StringUtil.fillString("" + number, 12, '0', true));
		}
		else
		{
