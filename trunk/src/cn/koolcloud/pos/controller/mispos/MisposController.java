...
public class MisposController extends BaseHomeController implements View.OnClickListener {
...
//		mHandler.sendEmptyMessageDelayed(SEND_DELAYED_COMMAND_HANDLER, 1000);
	...
	private void checkTranTypeFlow() {
		if (deliveryTranType.equals(SALE_TRAN_CONSTANT)) {
			if (!TextUtils.isEmpty(amount)) {
				...
				Intent mIntent = new Intent(MisposController.this, TransAmountController.class);
				mIntent.putExtra(KEY_REQUEST_FROM_MISPOS, KEY_REQUEST_FROM_MISPOS);
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case RECEIVE_BEAN_DATA_HANDLER:
				...
				if (responseCode.equals(MisposOperationUtil.RESPONSE_CODE_SUCCESS)) {
					...
					if (!TextUtils.isEmpty(beanData.getTransType())) {
						...
						if (beanData.getTransType().equals(MisposOperationUtil.TRAN_TYPE_CONSUMPTION)) {
							//FIXME: consumption write back to server and print receipt
						...
						if (beanData.getTransType().equals(MisposOperationUtil.TRAN_TYPE_CONSUMPTION_REVERSE)) {
							//FIXME: consumption reverse write back to server and print receipt
							new PrinterThread(beanData).start();
							new WriteBackThread(beanData).start();
							showOrderDetails(beanData);
						}
						...
						if (beanData.getTransType().equals(MisposOperationUtil.TRAN_TYPE_PRE_AUTHORIZATION)) {
							//FIXME: pre authorization write back to server and print receipt
							new PrinterThread(beanData).start();
							new WriteBackThread(beanData).start();
							showOrderDetails(beanData);
						}
						...
						if (beanData.getTransType().equals(MisposOperationUtil.TRAN_TYPE_PRE_AUTHORIZATION_REVERSE)) {
							//FIXME: pre authorization reverse write back to server and print receipt
							new PrinterThread(beanData).start();
							new WriteBackThread(beanData).start();
							showOrderDetails(beanData);
						}
						...
						if (beanData.getTransType().equals(MisposOperationUtil.TRAN_TYPE_PRE_AUTHORIZATION_COMPLETE)) {
							//FIXME: pre authorization complete write back to server and print receipt
							new PrinterThread(beanData).start();
							new WriteBackThread(beanData).start();
							showOrderDetails(beanData);
						}
						...
						if (beanData.getTransType().equals(MisposOperationUtil.TRAN_TYPE_PRE_AUTHORIZATION_COMPLETE_REVERSE)) {
							//FIXME: pre authorization complete reverse write back to server and print receipt
							new PrinterThread(beanData).start();
							new WriteBackThread(beanData).start();
							showOrderDetails(beanData);
						}
	...
	private void showOrderDetails(MisposData beanData) {
		...
		order_detail_tv_transAmount.setText(AppUtil.formatAmount(Long.parseLong(beanData.getAmount())));
		order_detail_tv_transDate.setText(MisposOperationUtil.parseDateTimeString(beanData.getTranDate(), beanData.getTranTime()));
		order_detail_tv_transType.setText(beanData.getTranTypeName());
		order_detail_tv_payType.setText(getResources().getString(R.string.mispos_str_mispos));
		order_detail_tv_orderId.setText(beanData.getRefNo());
		order_detail_tv_orderStatus.setText(getResources().getString(R.string.mispos_tran_success));
...
//		dismissLoading();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == GET_AMOUNT_REQUEST_CODE && resultCode == TransAmountController.RESULT_CODE_AMOUNT) {
	 ...
	 * <p>Description: The daemon thread to receive data </p>
	...
	/**
	 * <p>Title: MisposController.java </p>
	 * <p>Description: printer thread</p>
	 * <p>Copyright: Copyright (c) 2014</p>
	 * <p>Company: KoolCloud</p>
	 * @author 		Teddy
	 * @date 		2014-7-2
	...
	class WriteBackThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				req.put("transTime", beanData.getTranDate() + beanData.getTranTime());//M
