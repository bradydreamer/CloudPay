		...
		Thread androidHandlerThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				if ("alert".equals(androidHandlerName)){
					...
					clientEngine.netConnectInMainThread(jsonObjData, callBackId);
				} else if ("saveLocal".equals(androidHandlerName)) {
					clientEngine.saveLocal(jsonObjData, callBackId);
					...
					clientEngine.callBack(callBackId, clientEngine.getSystemInfo());
